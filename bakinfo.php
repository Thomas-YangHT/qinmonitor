...
<?php
     function ShowTable($table_name) {
        ...
        $sql="select `date`,b.hostname,a.ip,a.filename,a.count,a.space from $table_name as a inner join hostip as b on trim(a.ip)=trim(b.ip) order by filename desc,ip";        
        $res=mysql_query($sql,$conn);
        $rows=mysql_affected_rows($conn);//获取行数
        $colums=mysql_num_fields($res);//获取列数

        echo "<div style='margin:0 auto;align:center;font-size:22px;font-family:SimHei;width:800px'>".'欢迎您使用备份信息查询，查询的数据如下：(共计:'.$rows.'行'.$colums.'列)'."<a href='http://192.168.100.21/bakdetail.txt' target='right' >Detail bakinfo</a></div>\n";
